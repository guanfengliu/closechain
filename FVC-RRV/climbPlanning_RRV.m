function problem = climbPlanning_RRV(map, max_iter, is_benchmark, rand_seed, variant)
...
% consecutive hold points using RRV algorithm
...
function [path_j, path_c, narrow_topo, smooth_j, smooth_c, narrow_samples_j, narrow_samples_c,...
  ...
  % number of RRV samples
  numRRVSamples = 0;
  % number of RRV samples getting onto tree
  numRRVSamplesOnTree = 0;
  numRRVSamplesOnTreeNarrow = 0;
  numRRVSamplesOnTreeNarrowTopo = 0;
  numRRVSamplesRejected = 0;
  ...
  while ~reach_goal && total_iter < CONF.total_iter
    ...
    for i=1:num_samples,
       ...
       if ~strcmp(status, "Trapped")
         ...
         [project_samples, status, numCollision, numRejected] = problem.rrvSample(sample, nearest_node);
          numRejectedNodes = numRejectedNodes + numRejected + 1;
          numRRVSamplesRejected = numRRVSamplesRejected + numRejected;
          numCollisionChecks = numCollisionChecks +numCollision;
          numRRVSamples = numRRVSamples + size(project_samples, 2);
          for j=1:size(project_samples, 2)
              sample = project_samples(:,j);
             [nearest_node, nearest_node_ind] = problem.nearest(sample);
             [new_sample, status, numCollisionChecking] = problem.connect(nearest_node, sample);
              numCollisionChecks = numCollisionChecks + numCollisionChecking;
              % for close chain, the usual steering might be fails
              if ~strcmp(status, "Trapped")
                  numRRVSamplesOnTree = numRRVSamplesOnTree + 1;
                 [minClearance, isNarrow] = problem.calNarrowNess(new_sample);
                 if isNarrow
                    numNarrowOnTree = numNarrowOnTree + 1;  % these are narrow points on trees
                    numRRVSamplesOnTreeNarrow = numRRVSamplesOnTreeNarrow + 1;
                    if topo
                      narrow_samples_j = [narrow_samples_j, new_sample];
                      new_node_c = problem.generate_link_position(new_sample);
                      narrow_samples_c = [narrow_samples_c, new_node_c];
                      numNarrowOnTreeFromTopo = numNarrowOnTreeFromTopo + 1;
                      numRRVSamplesOnTreeNarrowTopo = numRRVSamplesOnTreeNarrowTopo + 1;
                    end
                 end 
                 [new_sample_ind, succ] =problem.insert_node(nearest_node_ind, new_sample, isNarrow && topo);
                 if ~succ
  ...
  fprintf("number of RRV samples =%d \n", numRRVSamples);
  fprintf("number of RRV samples on Tree = %d \n", numRRVSamplesOnTree);
  fprintf("number of RRV narrow samples on Tree = %d \n", numRRVSamplesOnTreeNarrow);
  fprintf("number of RRV narrow samples on Tree coming from topological component %d \n", numRRVSamplesOnTreeNarrowTopo);
  fprintf("number of rejected RRV samples =%d \n", numRRVSamplesRejected);
