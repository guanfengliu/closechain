...
obst.obstacle(3).normal_vec = normal_vec;
obst.obstacle(4).normal_vec = normal_vec;
...
[mpData] = preprocessCloseChainGen(mpData);
...
[startc,endc]=GenerateValidStartEndNarrowCfg(mpData, pos_left,pos_right,4,6,5,7,numPair);
...
mpData.nobst_samples=200; %10000; %100; %% how many random samples for each compatible fragment for each obstacle
...
mpData.nonbsamples=200; %3000; % 200; %% how many regular non boundary samples
...
% call PRM algorithm
[outMpData] = PRM(outMpData, @()(RandomSampleComp(mpData)), ...
                  @(x,y)(SimpleLocalPlannerClosedChain(x,y,mpData)),...
                  mpData.neighbors);
...
for i=1:numPair
  ...
  drawStartGoalConfigPolyObst(mpData,start_config, end_config,fig_hnd,fig_id);
  % Add start, end nodes, with reordered link length set, floatlinks, 
  roadmap2 = [];
  mpData.neighbors = 100;
  roadmap2 = AddNode2PRM(start_config,outMpData.kdtree, roadmap, @(x,y)(SimpleLocalPlannerClosedChain(x,y,mpData)), mpData.neighbors);
  roadmap2 = AddNode2PRM(end_config,outMpData.kdtree, roadmap2, @(x,y)(SimpleLocalPlannerClosedChain(x,y,mpData)), mpData.neighbors);
  SS=roadmap2.edges(:,1)';
  TT=roadmap2.edges(:,2)';
  ...
  success=drawAndSavePath(mpData,roadmap2, route, pathfileName,fig_id,start_config,end_config);
end
