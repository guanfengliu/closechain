function [start_config,end_config]=GenerateValidStartEndNarrowEff(linklength,thickNess,CollisionVarCritRadiusL,CollisionVarCritRadiusR,polyObst, obst, pos_left,pos_right,j_start,j_end, numPair)
...
rightChain = linklength(j_start+1:DOF-1);
...
while count < numPair && fail_count < MAX_FAILS
   left_config = [];
   left_config=RandomLoopGeneratorPoly(leftChain,CollisionVarCritRadiusL,CollisionVarCritRadiusR,false,false,1,InitAngleL);
   right_config = [];
   right_config=RandomLoopGeneratorPoly(rightChain,CollisionVarCritRadiusL,CollisionVarCritRadiusR,false,true,1,InitAngleR);
   if size(left_config,2) > 0 && size(right_config,2) > 0 %% if generate a valid sample
      sample = [left_config(1:j_start,1);right_config(1:DOF-j_start-1,2);pi];  
...
while count < numPair && fail_count < MAX_FAILS
   ...
   right_config=RandomLoopGeneratorPoly(rightChain,CollisionVarCritRadiusL,CollisionVarCritRadiusR,false,true,1,InitAngleR);
   if size(left_config,2) > 0 && size(right_config,2) > 0 %% if generate a valid sample
      sample = [left_config(1:j_end,1);right_config(1:DOF-j_end-1,2);pi];  
      drawConfig(linklength, sample, obst, 0.01);
      fv = closedchainthick (linklength,sample,thickNess);
      if (~CollisionCheck(fv, polyObst))
         count = count + 1;
