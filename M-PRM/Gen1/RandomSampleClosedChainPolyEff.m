function [out, bound_samples, obst_samples] = RandomSampleClosedChainPolyEff(linklength,thickness,...
               ...
               %% compute the intersection of the circle of l1 based at (0,0) and the circle of l2 based at pt
                 ...
                 if j == DOF -3   %% link DOF-2 touch the obstacle
                     ...
                     if j==DOF-2  %% link DOF-1 touch the obstacle
                        ...
                        [newConfig]=MidLinkRandomLoopGeneratorEff(linklength,CollisionVarCritRadiusL,CollisionVarCritRadiusR, vert, angle_intv, pt,j+1,k,peri, nobst_samples);  %% collision loop closure generator, not this is different from regular loop closure generator
                     end
                 end
              end
          end
          ...
          if noSamp > 0
             if j==0 | j==1
                 ...
                 for ddd=1:noSamp,
                   ...
                   [rightConfig1] = TraceObstHomotopy(linklength, CollisionVarCritRadiusL,CollisionVarCritRadiusR, 1,...
                                                      vert, angle_intv,...
                                                      start_link_index, start_vert_index, leftEnd(:,ddd),rightEnd(:,ddd), peri,ccw);
             ...
             if j==DOF-3 | j==DOF-2
                ...
                for ddd=1:noSamp,
                  ...
                  b_sam = [];
                 %% backward random loop generator
                  [leftConfig,b_sam]=RandomLoopGeneratorPoly(newlinklength,CollisionVarCritRadiusL,CollisionVarCritRadiusR,true,false,nobst_samples,initAngle(ddd));
                  % merge configure together
                  leftConfig = [leftConfig,b_sam];
                  numLeft=size(leftConfig,2);
                  if numLeft > 0 
