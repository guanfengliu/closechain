classdef FNClimbRobot_RRV < handle
...
%         obs_mat                 % special matrix for obstacle collision
    ...
    methods
        ...
        % holds (2*n matrix), 
        function this = FNClimbRobot_RRV(rand_seed, max_nodes, map, conf)
            rng(rand_seed);
        ...
        function [minDist, isNarrow, narrowVertices, narrowEdges, narrowLinks] = calNarrowNess(this, joint)
            narrowVertices=[]; narrowEdges=[]; narrowLinks = [];
            link_pos = this.generate_link_position(joint);
            minDist = inf;
            isNarrow = false;
            for i=1:this.num_links,
                [dist, isNarrowTemp, narrowVertex, narrowEdge] = this.determineNarrowDist(link_pos(i,:)', link_pos(i+1,:)');
        ...
        function [dist, isNarrow, narrowVertices, narrowEdges] = determineNarrowDist(this, pp1, pp2)
            narrowVertices = [];
            narrowEdges = [];
            ...
            for i=1:numNarrowVertex,
                narrow_vertex = this.narrow_vertices(i);
                qq = this.obstacle.coord(:, narrow_vertex);
        ...
        % @param: sample, the old sample which is in collision
        % @param: center, the closest neighbor in the tree, treated as
        % center in PCA and projections
        %parameter new_sample  the newly generated list of samples 
        % in the entrance and narrow passages
        function [new_sample, status, numCollision, numRejected] = rrvSample(this, sample, center)
          numCollision = 0;
          numRejected = 0;
          new_sample = [];
          status = [];
          % sample is already in collision
          % step 1: find nearest neighbor
          %[center, node_index] = this.nearest(sample);
           
          if this.conf.rrv_escape_estimatation_method == 0
            % next sample N samples near node, and estimate the princal
            % component (PCB)
            X = randn(this.conf.rrv_N,  this.num_links);
            X = X./sqrt(sum(X.^2, 2));
            R = nthroot(rand(this.conf.rrv_N, 1), this.num_links);
            % compute Q^t
            X = (X.* R) * this.conf.rrv_R + center'; % row by row
            
            % cX is collision set
            % fX is free set
            cX=[]; fX=[];
            for i=1:this.conf.rrv_N,
               numCollision = numCollision + 1; 
               if (this.CheckCollision(X(i, :)'))
                   cX = [cX; X(i, :)]; % row by row
               else
                   fX = [fX; X(i, :)]; % row by row
               end
            end
            
            % now compute pcb based ellipsoid of E_eig^{t, obst}
            [coeff, score, latent, ts, ex, mu] = pca(cX);
            
            %% now check whether there are any free samples inside
            % the E_eig^{t, obst}
            fXNarrow=[];
            for i=1:size(fX, 1)
               tmpVec = (fX(i,:) - mu) ./ (latent');
               sqN = sum(tmpVec.^2);
               if sqN <= 1.0
                   fXNarrow = [fXNarrow; fX(i,:)];
               end
            end
            % if Q^{t,fee} is empty
            if size(fXNarrow, 1)==0
                status = 'Front'; % front of convex obstacle
            else
              % now check if nearest point lies inside of the ellipsoid
              % sum(((center - mu)./ (latent')).^2) << 1
              unifyVec = (center' - mu) ./ (latent');
              sqN = sum(unifyVec.^2);
              if sqN <= 1.0  % then status InsideNarrow
                 status = 'InsideNarrow'; 
              else
                 status = 'EntranceNarrow';
              end
            end
            
          
            proj_ind = find(latent > this.conf.rrv_min_r);
            if length(proj_ind) > 0
              % get projection matrix
              A =  coeff(:, proj_ind);
              % now perform projection of sample
              tmp_sample = center + A* inv(A'*A) * A' * (sample - center);
              numCollision = numCollision + 1;
              if ~this.CheckCollision(tmp_sample)
                 new_sample = [new_sample, tmp_sample];
              else
                 numRejected = numRejected + 1;
              end
            end
             % now for each status, call different samples routine   
            if strcmp(status, 'EntranceNarrow')
               tmp_sample = (mean(fXNarrow))';
               if ~this.CheckCollision(tmp_sample)
                 new_sample = [new_sample, tmp_sample];
               else
                 numRejected = numRejected + 1;
             ...
             if strcmp(status, 'InsideNarrow')
               [coeff, score, latent, ts, ex, mu] = pca(fXNarrow);
               proj_ind = find(latent > this.conf.rrv_min_r);
               if length(proj_ind) > 0
                 B = coeff(:, proj_ind);
                 tmp_sample = center + B* inv(B'*B) * B' * (sample - center);
               else
                 tmp_sample = fXNarrow(1,:)';
               end
               tmp = center + ConvertNormal(tmp_sample - center) * [1:1:this.conf.rrv_narrow_numSamples-1]/this.conf.rrv_narrow_numSamples; 
               for i=1:size(tmp,2)
                   tmp1 = tmp(:,i);
                   numCollision = numCollision + 1;
                   if ~this.CheckCollision(tmp1)
                     new_sample = [new_sample, tmp1];
                   else
                     numRejected = numRejected + 1;
                   end
               end
          ...
          else  % Jacobian method
             % step 1, check if nearest node has some link in medium axis
             [minDist, isNarrow, narrowVertices, narrowEdges, narrowLinks] = this.calNarrowNess(center);
             if (isNarrow)
                ...
                for i=1:numNarrowLinks,
                   ll_vector = this.tree.linkLengthVec(start_link_index:narrowLinks(i))';
                   jt_vector = center(start_link_index:narrowLinks(i))';
                   % jacobian matrix for the link segment form
                   % start_link_index to narrowLinks(i) only
                   tJ = [ll_vector.*cos(jt_vector); ll_vector.*sin(jt_vector)];
                   ...
                   % of narrowLinks(i)
                   zero1 = zeros(1, this.num_links - narrowLinks(i));
                   zero2 = [zero1;zero1];
                ...
                tmp = center + ConvertNormal(center - tmp_sample) * [1:1:this.conf.rrv_narrow_numSamples-1]/this.conf.rrv_narrow_numSamples; 
                for i=1:size(tmp,2)
                   tmp1 = tmp(:,i);
                   numCollision = numCollision + 1;
                   if ~this.CheckCollision(tmp1)
