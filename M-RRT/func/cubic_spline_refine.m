function [ yy ] = cubic_spline_refine( x, y, xx)
...
for ind = 1 : x_num-2
    if(ind+1 < x_num)
        ...
        b(ind) = 3 * (ConvertNormal(y(ind+2) - y (ind+1))/h(ind+1) - ConvertNormal(y(ind+1) - y(ind))/ h(ind));
    end
end
...
for ind = 1:x_num-2
    ...
    b_i(ind) = ConvertNormal(y(ind+1) - y(ind)) / h(ind+1) - h(ind+1)/3 * (c_i(ind+1) + 2 * c_i(ind));
end
if x_num >= 2
  d_i(x_num-1) = -c_i(x_num-1) / (3 * h(x_num-1));
  b_i(x_num-1) = ConvertNormal(y(x_num) - y(x_num-1)) / h(x_num-1) - h(x_num-1)/3 * (2 * c_i(x_num-1));
end
